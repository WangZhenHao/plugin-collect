...
function BoyAnimate(params) {
  ...
  this.mouseScroll();
  this.scrollEvent();
  // alert(this.prefixStyle('animationDuration'))
}
...
BoyAnimate.prototype = {
  prefixStyle: function(style) {
    ...
    if(this.prefix === 'standard') {
      return style;
    }

    return this.prefix + style.charAt(0).toUpperCase() + style.substr(1);
  },
/**
 * 执行滚动条滚动事件
 ...
 */
  ...
  */
  removeAniamte: function(index) {
    ...
    if(cls.indexOf(attribute) > -1) {
	  ...
	  el.style[this.prefixStyle('animationDuration')] = "";
	  el.style[this.prefixStyle('animationDelay')] = "";
    }
  },
  /**
   * 添加各种动画的属性
  ...
  */
  toAnimate: function(index) {
  	var el = this.arrElment[index];
  	if(el.style[this.prefixStyle('animationDuration')]) {
  	  return;
  	}
  	el.className += " animate " + this.getAttribute(el, "boy-animate-effect");
  	el.style[this.prefixStyle('animationDuration')] = this.getAttribute(el, "boy-animate-duration");
  	el.style[this.prefixStyle('animationDelay')] = this.getAttribute(el, "boy-animate-delay");
  },
  /**
  * 获取节点的属性
